...
sub isPrime{
    ...
    return 0 if $num <= 1;
    return 1 if $num == 2;
    for(my $i = 3 ; $i * $i <= $num1 ; $i += 2){        #Loop from 2 till a number which is less-than or equal to square root of given number; it is guranteed that all the factors of number will be in this range
